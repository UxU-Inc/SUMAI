...
class LoginMain extends Component {
  handleChckSignupEmail = (email) => {
    return axios
      .post('/api/account/checkSignupEmail', { email })
      .then((res) => {
        return { success: true };
      })
      .catch((err) => {
  ...
  handleLogin = (email, password) => {
    return this.props.loginRequest(email, password).then(() => {
      if (this.props.login.status === 'SUCCESS') {
        ...
        let domainName;
        if (domainIndex === -1) domainName = window.location.hostname;
        // .을 못 찾은 경우 그대로 씀 -> localhost
  ...
  handleSNSLogin = (type) => {
    return this.props.snsloginRequest(type).then((email) => {
      if (this.props.login.status === 'SUCCESS') {
        let domainIndex = window.location.hostname.indexOf('.'); // ex) asdf.good.com -> 5 (.의 위치)
        let domainName;
        if (domainIndex === -1) domainName = window.location.hostname;
